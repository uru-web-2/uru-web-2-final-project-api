...
import {
    ...
    CREATE_GET_DOCUMENT_ID_BY_ARTICLE_ID_PROC,
    CREATE_GET_DOCUMENT_ID_BY_BOOK_ID_PROC,
    CREATE_GET_DOCUMENT_ID_BY_MAGAZINE_ISSUE_ID_PROC,
    CREATE_GET_DOCUMENT_ID_BY_THESIS_ID_PROC,
    ...
    CREATE_IS_DOCUMENT_REVIEW_ID_VALID_PROC, CREATE_IS_LANGUAGE_ID_VALID_PROC,
    ...
    for (const objectName of Object.keys(module.objects)) {
        ...
        for (const methodName of Object.keys(object.methods)) {
            ...
            try {
                ...
                const profilesIDToAdd = allowedProfilesID.filter((profileID) => !currentAllowedProfilesID.includes(
                    profileID))

                // Get the profiles ID to remove
                const profilesIDToRemove = currentAllowedProfilesID.filter((profileID) => !allowedProfilesID.includes(
                    profileID))
...
export default async function migrate() {
    ...
    await DatabaseManager.runTransaction(async (client) => {
        ...
        try {
            for (const [index, query] of Array.from([
                CREATE_GET_ALL_MODULES_FN,
                CREATE_GET_ALL_OBJECTS_FN,
                CREATE_GET_ALL_METHODS_FN,
                CREATE_GET_ALL_USER_PROFILES_FN,
            ...
            throw new Error(`Error creating function ${currentIndex+1}: ${error}
    ...
    await DatabaseManager.runTransaction(async (client) => {
        ...
        try {
            for (const [index, query] of Array.from([
                ...
                CREATE_IS_BOOK_COPY_LOAN_ID_VALID_PROC,
                CREATE_IS_ARTICLE_JURY_MEMBER_PROC,
                CREATE_IS_LANGUAGE_ID_VALID_PROC,
                CREATE_GET_USER_EMAIL_ID_BY_USER_EMAIL_PROC,
                CREATE_GET_COUNTRY_ID_BY_NAME_PROC,
                CREATE_CREATE_USER_PERSONAL_DOCUMENT_PROC,
                CREATE_REMOVE_USER_PERSONAL_DOCUMENT_PROC,
            ...
            throw new Error(`Error creating stored procedure ${currentIndex+1}: ${error}
            
Stored procedure: ${currentQuery}`)
        }
    })
